"""
Issue APIテストコード - Issue #26 (タスク#025)

AT-101~AT-117のAPIテストケース仕様に基づく包括的テストスイート
Issue API (tracker/api/issue_api.py) の全エンドポイントをテスト
...
- PATCH  /api/issues/{id}/assignee       → assign_issue
"""
import json
...
class IssueAPITest(TestCase):
    """
    Issue API包括テストクラス
    AT-101~AT-117の設計仕様に準拠した17個のテストケースを実装
    """
    ...
    def test_list_issues_success(self):
        ...
        self._create_issue_via_api(
            self._create_test_issue_data(title='Issue 1')
        )
        self._create_issue_via_api(
            self._create_test_issue_data(title='Issue 2')
        )
